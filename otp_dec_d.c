/* James Whiteley IV
 * OTP - server decryption
 * 2017-8-7
 * Description: runs in background. Receives text file and key from otp_enc and decodes the file,
 * 	then sends back to otp_dec. 
 * usage:
 * otp_enc_d [port] &
 */
#include <stdio.h>
...
char convertInt(int i) {
	...
	else { //for all other chars, add 65 to get ascii
		i += 65;	
		char c = i;
		return c;
	}
...
//decrypt the plaintext using key 
void decrypt(char *buffer) {
	...
	while(c) {
		...
		cdec = convertChar(c); //get integer form of char in buffer 
		kdec = convertChar(k);
		cdec -= kdec; //subtract key value to plaintext char
		if (cdec < 0)  //if less than 0, add 27 
			cdec += 27;  

		buffer[i] = convertInt(cdec); //convert int back to char	
		i++;
		c = plaintext[i]; //get next position
		k = key[i];
	}
...
int main(int argc, char *argv[])
{
	...
	//child then decrypts file and writes back the cipher text to the otp_enc process connected to same communication socket
	//original server daemon (parent?) continues to listen for new connections (only the child decrypts and sends back)
	...
	while(1) {
		...
		if (pid == 0) { //child process
			...
			char decD[2];
			int decRead = recv(establishedConnectionFD, &decD, sizeof(decD), 0); 
			if (strncmp(decD, "D", 1) == 0) {
				char correctConn[2] = "T";
				int decWrite = send(establishedConnectionFD, &correctConn, sizeof(correctConn), 0); 
			}
			else {
			...
			int reqSize = ntohl(recSize); //convert to int
			...
			while (cur < bytesLeft) {
				...
				cur += charsWritten;
				bytesLeft -= charsWritten; //remove the amount of bytes written
			}
