/* James Whiteley IV
 * OTP - client decryption
 * 2017-8-7
 * Description: This program connects to otp_dec_d and asks it to do OTP style decyrption.
 * usage:
 * otp_dec [plaintext file name to decrypt] [decryption key file] [port]
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char *argv[]) {
	...
	if (argc < 4) {
		fprintf(stderr, "USAGE: otp_dec [plaintext] [keyfile] [port]\n");
		exit(1);
	}

	char *plaintext = argv[1]; //get plaintext file 
	...
	//make sure connecting to decryptor not encryptor, exit(2)
	char correctConn[2];
	char decD[2] = "D";
	int decWrite = send(socketFD, &decD, sizeof(decD), 0);
	int decRead = recv(socketFD, &correctConn, sizeof(correctConn), 0);	
	...
	while (cur < bytesLeft) {
		...
		cur += charsWritten;
		bytesLeft -= charsWritten; //remove num bytes written
	}
	...
	do {
		...
		pos += charsRead;
		bytesLeft -= charsRead;
	} while (bytesLeft > 0); //while there are still bytes to be read in
